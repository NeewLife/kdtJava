...
public class Cat {
	...
	public Cat(String name, String kind) {
		...
		super();	// super = 부모 = 이놈의 부모는 누군가? 특별히 Cat의 부모가 누구인지 명시 안됨.
			// 부모의 부모의 부모의... 더이상 부모가 없는 최종 조상. Object
			// super는 그렇다 치고 super()는 뭔가?	Object();
			// 최종조상의 인스턴스 하나를 생성.
			// 왜? 나라는 존재를 생성하기 위해서. 조상이 없는 나는 존재할 수가 없기 때문에
			// 부모 인스턴스를 생성해주는 super()가 없으면 .class 파일로 변환될 때 자동생성
		this.name = name;
		this.kind = kind;
	}
